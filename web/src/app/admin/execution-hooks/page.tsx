...
export default function ExecutionHooksPage() {
  ...
  const t = useTranslations('adminExecutionHooks')
        ...
        {showCreate && (
          <form onSubmit={handleCreate} className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-4 mb-6 space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label htmlFor="hook-name" className="block text-sm font-medium text-slate-300 mb-1">{t('form.name')}</label>
              ...
              <div>
                <label htmlFor="hook-desc" className="block text-sm font-medium text-slate-300 mb-1">{t('form.description')}</label>
                <input id="hook-desc" type="text" value={description} onChange={(e) => setDescription(e.target.value)}
                  placeholder={t('form.descriptionPlaceholder')}
                  className="w-full px-3 py-2 border border-slate-600 rounded-lg bg-slate-700 text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
              </div>
              <div>
                <label htmlFor="hook-point" className="block text-sm font-medium text-slate-300 mb-1">{t('form.hookPoint')}</label>
              ...
              <div>
                <label htmlFor="hook-priority" className="block text-sm font-medium text-slate-300 mb-1">{t('form.priority')}</label>
            ...
            <div>
              <label htmlFor="hook-script" className="block text-sm font-medium text-slate-300 mb-1">
                {t.rich('form.scriptLabel', { code: (chunks) => <code>{chunks}</code> })}
              </label>
              ...
              <p className="mt-1 text-xs text-slate-500">{t('form.scriptHelp')}</p>
            </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <SortableHeader label={t('columns.hookPoint')} sortKey="point" sortState={sortState} onSort={toggleSort} className="hidden sm:table-cell" />
                  <SortableHeader label={t('columns.enabled')} sortKey="enabled" sortState={sortState} onSort={toggleSort} className="hidden md:table-cell" />
                  <SortableHeader label={t('columns.priority')} sortKey="priority" sortState={sortState} onSort={toggleSort} className="hidden md:table-cell" />
                  <SortableHeader label={t('columns.workspaces')} sortKey="workspaces" sortState={sortState} onSort={toggleSort} className="hidden lg:table-cell" />
                </tr>
              </thead>
                  ...
                  <tr key={h.id} className="hover:bg-slate-700/20 transition-colors cursor-pointer"
                    ...
                    <td className="px-4 py-3">
                      ...
                      {!h.attributes.enabled && (
                        <span className="md:hidden mt-1 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-700 text-slate-400">{t('badges.disabled')}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      {h.attributes.enabled ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-900/50 text-green-300">{t('badges.enabled')}</span>
                      ) : (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-700 text-slate-400">{t('badges.disabled')}</span>
                      )}
                    </td>
