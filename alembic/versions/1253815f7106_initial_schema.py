...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("allow_labels", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("allow_names", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("deny_labels", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_table(
        ...
        sa.Column("email", sa.String(255), primary_key=True),
        sa.Column("role_name", sa.String(63), sa.ForeignKey("roles.name", ondelete="CASCADE"), primary_key=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    ...
    op.create_table(
        "agent_pool_tokens",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("pool_id", UUID(as_uuid=True), sa.ForeignKey("agent_pools.id", ondelete="CASCADE"), nullable=False),
        sa.Column("token_hash", sa.String(64), unique=True, nullable=False),
        sa.Column("description", sa.String(255), nullable=False, server_default=""),
    ...
    op.create_table(
        ...
        sa.Column("runner_definitions", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    ...
    op.create_table(
        ...
        sa.Column("github_installation_id", sa.Integer(), nullable=False, server_default=sa.text("0")),
    ...
    op.create_table(
        ...
        sa.Column("agent_pool_id", UUID(as_uuid=True), sa.ForeignKey("agent_pools.id", ondelete="SET NULL"), nullable=True),
        ...
        # VCS
        sa.Column("vcs_connection_id", UUID(as_uuid=True), sa.ForeignKey("vcs_connections.id", ondelete="SET NULL"), nullable=True),
        ...
        # Drift detection
        sa.Column("drift_detection_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("drift_detection_interval_seconds", sa.Integer(), nullable=False, server_default=sa.text("86400")),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("module_id", UUID(as_uuid=True), sa.ForeignKey("registry_modules.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("provider_id", UUID(as_uuid=True), sa.ForeignKey("registry_providers.id", ondelete="CASCADE"), nullable=False),
        sa.Column("version", sa.String(63), nullable=False),
        sa.Column("gpg_key_id", UUID(as_uuid=True), sa.ForeignKey("gpg_keys.id"), nullable=True),
        sa.Column("protocols", JSONB(), nullable=False, server_default=sa.text("'[\"5.0\"]'::jsonb")),
        sa.Column("shasums_uploaded", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("shasums_sig_uploaded", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("version_id", UUID(as_uuid=True), sa.ForeignKey("registry_provider_versions.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("hostname", "namespace", "type", "version", "os", "arch", name="uq_cached_provider_packages"),
    )
    op.create_index("ix_cached_provider_packages_lookup", "cached_provider_packages", ["hostname", "namespace", "type"])
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("variable_set_id", UUID(as_uuid=True), sa.ForeignKey("variable_sets.id", ondelete="CASCADE"), nullable=False),
        sa.Column("key", sa.String(255), nullable=False),
        sa.Column("value", sa.Text(), nullable=False, server_default=""),
        sa.Column("description", sa.Text(), nullable=False, server_default=""),
        sa.Column("category", sa.String(20), nullable=False, server_default="terraform"),
    ...
    op.create_index("ix_variable_set_variables_set_id", "variable_set_variables", ["variable_set_id"])

    op.create_table(
        "variable_set_workspaces",
        sa.Column("variable_set_id", UUID(as_uuid=True), sa.ForeignKey("variable_sets.id", ondelete="CASCADE"), primary_key=True),
    ...
    op.create_table(
        ...
        sa.Column("status", sa.String(20), nullable=False, server_default="pending"),
        sa.Column("auto_queue_runs", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column("speculative", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ix_configuration_versions_workspace_id", "configuration_versions", ["workspace_id"])
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("workspace_id", UUID(as_uuid=True), sa.ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False),
        sa.Column("configuration_version_id", UUID(as_uuid=True), sa.ForeignKey("configuration_versions.id", ondelete="SET NULL"), nullable=True),
        ...
        sa.Column("resource_cpu", sa.String(20), nullable=False, server_default="1"),
        sa.Column("resource_memory", sa.String(20), nullable=False, server_default="2Gi"),
        sa.Column("pool_id", UUID(as_uuid=True), sa.ForeignKey("agent_pools.id", ondelete="SET NULL"), nullable=True),
        sa.Column("listener_id", UUID(as_uuid=True), sa.ForeignKey("runner_listeners.id", ondelete="SET NULL"), nullable=True),
        sa.Column("error_message", sa.Text(), nullable=False, server_default=""),
        # Drift detection
        sa.Column("is_drift_detection", sa.Boolean(), nullable=False, server_default=sa.text("false")),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("workspace_id", UUID(as_uuid=True), sa.ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False),
        sa.Column("source_workspace_id", UUID(as_uuid=True), sa.ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    ...
    op.create_table(
        ...
        sa.Column("email_addresses", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("delivery_responses", JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    op.create_index("ix_notification_configurations_workspace_id", "notification_configurations", ["workspace_id"])
    ...
    op.create_table(
        ...
        sa.Column("enforcement_level", sa.String(20), nullable=False, server_default="mandatory"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    ...
    op.create_table(
        "task_stage_results",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("task_stage_id", UUID(as_uuid=True), sa.ForeignKey("task_stages.id", ondelete="CASCADE"), nullable=False),
        sa.Column("run_task_id", UUID(as_uuid=True), sa.ForeignKey("run_tasks.id", ondelete="SET NULL"), nullable=True),
        sa.Column("status", sa.String(30), nullable=False, server_default="pending"),
        sa.Column("message", sa.Text(), nullable=False, server_default=""),
